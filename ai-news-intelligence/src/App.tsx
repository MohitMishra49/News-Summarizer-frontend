...
import { countWords, loadHistory, addToHistory, removeFromHistory } from './lib/utils';
...
export default function App() {
  ...
  const handleSummarize = async () => {
    ...
    try {
      const generated = await summarizeArticle(article);
      const processingTime = Date.now() - startTime;
      const originalWordCount = countWords(article);
      ...
      const historyItem: HistoryItem = {
        ...
        timestamp: Date.now(),
        articlePreview: article.slice(0, 100) + (article.length > 100 ? '…' : ''),
